...
public class StateController {
    ...
    public StateController(StateStore stateStore) {
    ...
    public Response getFrameworkId() {
        try {
            JSONObject configObj = new JSONObject(stateStore.fetchFrameworkId());
    ...
    public Response getTaskNames() {
        try {
            JSONArray taskArray = new JSONArray(stateStore.fetchTaskNames());
            return Response.ok(taskArray.toString(), MediaType.APPLICATION_JSON).build();
        } catch (Exception ex) {
    ...
    public Response getTaskInfo(@PathParam("taskName") String taskName) {
        try {
            ...
            JSONObject configObj = new JSONObject(stateStore.fetchTask(taskName));
            return Response.ok(configObj.toString(), MediaType.APPLICATION_JSON).build();
        } catch (Exception ex) {
    ...
    public Response getTaskStatus(@PathParam("taskName") String taskName) {
        try {
            logger.info("Attempting to fetch TaskInfo for task '{}'", taskName);
            JSONObject configObj = new JSONObject(stateStore.fetchStatus(taskName));
            return Response.ok(configObj.toString(), MediaType.APPLICATION_JSON).build();
        } catch (Exception ex) {
            // Warning instead of Error: Subject to user input
            logger.warn(String.format(
